...
namespace App\Modules\Platform\Money;

use Brick\Money\Currency;

/**
 * Money as integer minor units, converted to and from text with string arithmetic only: an amount never passes through a
 * float (CONTEXT.md non-negotiable #5). Scale = the currency's default fraction digits. Shared by every context.
 */
final class MinorUnits
{
    ...
    /** "-150.00" BDT → -15000; like fromMajor but a leading minus sign is allowed. */
    public static function fromSignedMajor(string $text, string $currency): ?int
    {
        $negative = str_starts_with($text, '-');
        $amount = self::fromMajor($negative ? substr($text, 1) : $text, $currency);
