...
final class PremiumRegisterQuery
{
     ...
     *     by_class: list<array{group: string, gross_minor: int, net_minor: int, tax_minor: int}>, by_branch: list<array{group: string, gross_minor: int, net_minor: int, tax_minor: int}>,
     ...
     *     branch_id: string, branch_code: string, agent_id: string|null, customer_id: string, currency: string, gross_minor: int, net_minor: int, tax_minor: int,
    ...
    public function register(string $entityId, CarbonImmutable $from, CarbonImmutable $to): array
    {
        ...
        foreach ($transactions as $t) {
            ...
            $byClass = self::accumulate($byClass, (string) $t->lob, $gross, $net, $tax);
            $byBranch = self::accumulate($byBranch, (string) $t->branch_code, $gross, $net, $tax);
        }
        ksort($byClass);
        ksort($byBranch);
    ...
    /**
     * @param array<string, array{group: string, gross_minor: int, net_minor: int, tax_minor: int}> $groups
     * @return array<string, array{group: string, gross_minor: int, net_minor: int, tax_minor: int}>
     */
    private static function accumulate(array $groups, string $group, int $gross, int $net, int $tax): array
    {
        $current = $groups[$group] ?? ['group' => $group, 'gross_minor' => 0, 'net_minor' => 0, 'tax_minor' => 0];
