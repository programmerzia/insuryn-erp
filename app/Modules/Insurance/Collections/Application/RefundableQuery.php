...
final class RefundableQuery
{
    ...
    public function refundable(string $entityId): array
    {
        $rows = DB::table('policies as p')->join('parties as h', 'h.id', '=', 'p.policyholder_party_id')->where('p.entity_id', $entityId)->where('p.status', 'cancelled')
