...
final class PayrollRunsPageController
{
    ...
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('people/payroll/Index', [
            'runs' => DB::table('payroll_runs')->where('entity_id', $entity['id'])->orderByDesc('period_year')->orderByDesc('period_month')->get()
    ...
    public function show(Request $request, string $run, ObjectHistory $history, SodGuard $sod): Response
    {
        ...
        return Inertia::render('people/payroll/Show', [
            ...
            'journals' => Inertia::defer(fn (): array => $history->accounting($journalIds), 'history'),
    ...
    public function payslips(Request $request): Response
    {
        ...
        $entity = PageSupport::entity();
        $money = fn (mixed $minor): string => PageSupport::money((int) $minor, $entity['currency']);
        $runId = is_string($request->query('run')) ? $request->query('run') : null;
        ...
        return Inertia::render('people/payslips/Index', [
            ...
            'payslips' => DB::table('payslips as p')->join('payroll_runs as r', 'r.id', '=', 'p.run_id')->join('employees as e', 'e.id', '=', 'p.employee_id')
                ...
                ->get(['p.id', 'p.number', 'p.run_id', 'r.period_year', 'r.period_month', 'r.status', 'e.id as employee_id', 'e.code', 'e.full_name', 'p.gross_minor', 'p.tax_minor', 'p.pf_employee_minor', 'p.net_minor'])
