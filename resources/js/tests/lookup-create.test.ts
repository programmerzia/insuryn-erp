...
import { blankCreateDraft, createPayload, lookupCreateConfig } from '@/lib/lookupCreate';

describe('inline create from a lookup (flow fix X8)', () => {
    it('creates customers and payees, and nothing else', () => {
        expect(lookupCreateConfig('customer')).toEqual({ noun: 'customer', endpoint: '/lookup/customer', roles: null });
    ...
    it('starts a customer as an individual and a payee as an organisation vendor, named from what was typed', () => {
        expect(blankCreateDraft(lookupCreateConfig('customer'), 'Nazmul')).toEqual({ display_name: 'Nazmul', kind: 'individual', tax_id: '', role: '' });
    ...
    it('sends an empty tax ID as null, the role only for payees, and the claim the payee is for', () => {
        ...
        expect(createPayload(customer, { display_name: 'A', kind: 'individual', tax_id: ' ', role: '' })).toEqual({ display_name: 'A', kind: 'individual', tax_id: null });
