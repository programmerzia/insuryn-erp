...
/** Toolbar period filter: "From … to …" or "As of …", keyboard dates or the calendar (the end cannot be before the start), applied as query parameters (kept in the URL). */
...
<template>
    <form class="ml-2 flex shrink-0 items-center gap-1.5 text-ui whitespace-nowrap text-ink-2" @submit.prevent="apply">
        <template v-if="asOf !== undefined">
            ...
            <DateInput id="filter-as-of" v-model="range.as_of" class="w-36" @update:model-value="apply" />
        </template>
        <template v-else>
            <label for="filter-from">From</label>
            <DateInput id="filter-from" v-model="range.from" class="w-36" :max="range.to || null" @update:model-value="apply" />
            <label for="filter-to">to</label>
            <DateInput id="filter-to" v-model="range.to" class="w-36" :min="range.from || null" @update:model-value="apply" />
        </template>
    </form>
</template>
