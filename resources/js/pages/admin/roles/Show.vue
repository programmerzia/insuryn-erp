...
const props = defineProps<{
    ...
    granted: string[];
    catalogue: { label: string; permissions: { code: string; label: string }[] }[];
            ...
            <form aria-labelledby="permissions-heading" @submit.prevent="save">
                ...
                <div class="grid gap-x-8 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">
                    <fieldset v-for="group in catalogue" :key="group.label" class="grid content-start gap-1.5">
                        ...
                        <label v-for="permission in group.permissions" :key="permission.code" class="flex items-center gap-2 text-ui" :title="permission.code">
                            <input v-model="form.permissions" type="checkbox" :value="permission.code" class="size-3.5 accent-accent" />
                            {{ permission.label }}
                        </label>
                    </fieldset>
                </div>
