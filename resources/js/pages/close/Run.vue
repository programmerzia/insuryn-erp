...
<template>
    <AppLayout :title="`Close ${run.period}`">
        <PageHeader eyebrow="Month-end close" :title="`Close ${run.period}`" :description="`Run ${run.status}; period ${run.period_status}. Tasks run once their dependencies are done or skipped.`">
            ...
            <Link href="/close" class="text-sm text-blueprint hover:underline">All periods</Link>
        </PageHeader>
        <FormBanner />
        <Table>
            ...
            <TableBody>
                <TableRow v-for="task in props.tasks" :key="task.id">
                    <TableCell class="font-mono text-ivory-dim">{{ task.order_no }}</TableCell>
                    <TableCell class="capitalize">{{ label(task.code) }}<p v-if="task.depends_on.length" class="text-xs text-ivory-dim normal-case">after {{ task.depends_on.map(label).join(', ') }}</p></TableCell>
                    <TableCell class="text-ivory-dim">{{ label(task.owner_role) }}</TableCell>
                    <TableCell><StatusBadge :status="task.status" /></TableCell>
                    <TableCell class="max-w-xs text-sm text-ivory-dim">{{ task.summary }}</TableCell>
