...
<template>
    <AppLayout :title="`Agent ${agent.code}`">
        ...
        <div class="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
            <Card><p class="text-xs text-ivory-dim uppercase">Earned</p><p class="mt-1 font-mono text-lg tabular-nums">{{ statement.totals.earned }}</p></Card>
            <Card><p class="text-xs text-ivory-dim uppercase">Clawed back</p><p class="mt-1 font-mono text-lg tabular-nums">{{ statement.totals.clawback }}</p></Card>
            <Card><p class="text-xs text-ivory-dim uppercase">Withheld</p><p class="mt-1 font-mono text-lg tabular-nums">{{ statement.totals.withholding }}</p></Card>
            <Card><p class="text-xs text-ivory-dim uppercase">Net</p><p class="mt-1 font-mono text-lg tabular-nums">{{ statement.totals.net }}</p></Card>
        </div>
        <Table>
            ...
            <TableBody>
                <TableRow v-for="entry in statement.entries" :key="entry.id">
                    <TableCell>{{ entry.earned_on }}</TableCell><TableCell>{{ entry.kind }}</TableCell><TableCell class="font-mono">{{ entry.policy_number }}</TableCell>
                    <TableCell class="text-right font-mono tabular-nums">{{ entry.base }}</TableCell><TableCell class="text-right">{{ entry.rate_percent ? entry.rate_percent + '%' : '' }}</TableCell>
                    <TableCell class="text-right font-mono tabular-nums">{{ entry.amount }}</TableCell><TableCell class="text-right font-mono tabular-nums">{{ entry.withholding }}</TableCell>
                    <TableCell class="text-right font-mono tabular-nums">{{ entry.net }}</TableCell><TableCell><StatusBadge :status="entry.status" /></TableCell>
                </TableRow>
