...
composer worker                      # queue worker (posting,batch,recon,default) and the scheduler together (GA-05); `composer dev` runs both too
...
| A-161 | H2 | The Bangla wording of risk field problems is not specified: ours ("{label} লিখুন।", "{label} বেছে নিন।", "কমপক্ষে {n} লিখুন।", "সর্বোচ্চ {n} লিখুন।", "{n} অক্ষরের মধ্যে রাখুন।", "ঝুঁকির বিবরণ দেখুন: …"), with Latin digits as on Bangla documents (A-104), shown to a user whose `locale` preference is Bangla. Verify with the customer. | `App\Http\Feedback\RiskProblems` (`ASSUMPTION A-161`); `QuotationWorkbenchTest`, `ProposalRiskDetailsTest`, `PolicyIssueFromProposalTest`. |
| A-162 | H3 | Design §5.5 does not say how a reopened claim ends when nothing more is paid, nor whether a recovery is taken while it is open again: a paid claim reopened (status `reserved` with a paid payment) is closed by `claim.close` like a paid claim, with no approval of its own (closing never had one), releasing the reserve above what was approved; a recovery is recorded on any claim with money paid, in any status, including a claim rejected after reopening. A reopened claim with nothing paid is still ended by paying or rejecting it. Verify with the customer's claims process. | `ClaimService::closable`, `recoverable` (`ASSUMPTION A-162`), `ClaimPageController::show` actions; `ClaimsTest`, `ClaimReservePropertyTest`. |
...
| A-171 | GA-01 | Who requests a refund is not specified: design §7.2 gives `receipt.refund_request` to no role. The Branch Manager template holds it (the manager cancels the policy); the Finance Manager and CFO keep `receipt.refund_release`, so the §7.3 rule request ✕ release holds and no template has both. **Verify** with the customer (some insurers have finance raise the refund voucher). Existing tenants' `branch_manager` role gets it by migration `2026_09_30_000071`. | `RoleTemplates` (branch_manager), migration `2026_09_30_000071_grant_refund_request_to_branch_manager`, `NextSteps::afterCancel`. |
| A-172 | GA-13 | Whether the accountant reads reports is not specified: §7.2 gives the Accountant no `reports.financial`, yet the role reconciles bank and suspense. The Accountant template holds it (read-only: trial balance, account activity, close checklist, reports, and the business screens that open for report readers); added to the accountant role only, not to the shared accountant list. Migration `2026_09_30_000073`. **Verify**. | `RoleTemplates` (accountant). |
| A-173 | GA-08 | When a queued accounting event counts as stuck is not specified: queued for more than 15 minutes (the worker normally posts within seconds). | `config/erp.php` `posting.stale_after_minutes` (env `ERP_POSTING_STALE_AFTER_MINUTES`), `StuckAccountingEvents` (`ASSUMPTION:`). |
| A-174 | GA-08 | Design §5.1 names `accounting.requeue_event` but gives it to no role: the Finance Manager template (and CFO, built on it) holds it. Requeue puts a failed event back to `queued` (failure reason cleared, the old one kept in the audit trail) and dispatches it; a stale queued event is dispatched again as it is; a posted or freshly queued event is refused (`EVENT_NOT_STUCK`). No SoD rule involves it. **Verify**. Migration `2026_09_30_000073`. | `RoleTemplates`, `StuckAccountingEvents::requeue`, `AccountingEventsPageController`. |
| A-175 | GA-12 | Which reports the claims desk reads is not specified: new permission `reports.claims` opens outstanding claims, claims paid and loss ratio (read-only; drill-down links other than to claims are removed for such a reader); Claims Officer and Claims Manager templates hold it. Registration warns, never refuses, when premium due by the date of loss is unpaid. **Verify**. Migration `2026_09_30_000074` adds the permission and grants it. | `ReportsPageController::CLAIMS_REPORTS`, `PermissionsSeeder`, `RoleTemplates`, `ClaimPolicyFacts`. |
| A-181 | GA-42 | The commission base on premium received is not specified by the customer (product owner decision D-70, **verify**, CQ-F8): agent commission is a percentage of the NET premium in each allocation, without VAT and stamp duty (Bangladesh practice; IDRA commission caps are on net premium). An installment is an equal slice of the gross premium of the transaction that billed it (the issue for installments 1..n, the k-th premium increase for installment n + k), so the net share of an allocation is allocation × net ÷ gross of that transaction, half-even; an allocation not to an installment uses the policy's current net ÷ gross. One setting for every scheme and Phase 1 plan (neither has a base field): `gross` pays on the cash allocated, as before. Written-premium and net-premium scheme rules on issue are unchanged. | `config/erp.php` `commission.premium_received_base` (env `ERP_COMMISSION_PREMIUM_RECEIVED_BASE`), `EarnCommissionOnAllocation::base`, `Policy\Application\PremiumNetShare`. |
| A-182 | GA-44 | The earning method of the demo and template products is not confirmed (product owner decision D-71, **verify**, CQ-D1): 1/365 per day on cover (`daily_365`) for the Part A demo products and the first product of the setup wizard; `monthly` stays available per product version and as the wizard default through config. | `config/erp.php` `setup.earning_method` (env `ERP_SETUP_EARNING_METHOD`), `SetupPageController::product`, `PartADemoSeeder`. |
| A-183 | GA-41 | The claim documents' content is not specified: the acknowledgement prints the claim number, policy, policyholder, product, period, date of loss, reported date and description, and no amount (the reserve is an estimate, not an offer); it can be printed for any claim. A discharge voucher is printed per claim payment once approved (approved, release requested, release awaiting approval or paid), numbered `<claim number>/P<n>` in the order payments were approved, naming the payee, the amount "in full and final settlement" and the claimant, witness and company signature lines; a payment waiting for approval or rejected has none (`DOCUMENT_OBJECT_NOT_READY`). Wording is the default template text (A-107). | `Claims\Application\Documents\ClaimAcknowledgementDocumentData`, `DischargeVoucherDocumentData`. |
| A-184 | GA-41 | Who prints claim documents is not specified: `document.generate` is added to the Claims Officer template (and the Claims Manager through it), so the claims desk prints the acknowledgement and discharge vouchers in the claim's branch. Existing tenants get it on the template roles by migration. | `RoleTemplates::CLAIMS_OFFICER`, migration `2026_09_30_000071_grant_claims_roles_document_generate`. |
| A-185 | GA-41 | Who may attach documents to a producer is not specified: holders of `agent.manage` (who create producers and record licences); anyone who opens the producer page (its area) may download them. | `App\Http\Distribution\ProducersPageController::attachDocument` / `downloadDocument` (`ASSUMPTION:`). |
| A-186 | GA-46 | Which outbox messages have a consumer is not specified: `JournalPosted`, `PeriodLocked`, `PeriodReopened` and `PeriodSoftLocked` announce a change and have no subscriber in this build, so the relay marks them relayed (delivered to nobody) instead of leaving them unrelayed forever; messages that wait for a Phase 2 consumer (`CommissionPayableToAp`, `CommissionPayrollEarning`, `DunningNoticeDue`, `ProducerLicenceExpiring`) stay unrelayed. A reconciliation rerun whose measured subledger and ledger balances equal the latest clean run of the period reuses that run instead of recording a duplicate. | `config/erp.php` `outbox.no_subscriber_types`, `OutboxRelayJob`, `ReconciliationService::run`. |
| A-187 | GA-47 | Which dimensions a tenant requires beyond the posting rules is not specified: branch, product and class (`lob`) on the policy, premium earning and claim events (POLICY_ISSUED, POLICY_ENDORSED, POLICY_CANCELLED, PREMIUM_EARNED, CLAIM_RESERVED, CLAIM_RESERVE_ADJUSTED, CLAIM_APPROVED, CLAIM_PAID, CLAIM_RECOVERED, CLAIM_CLOSED). The rows are seeded for the demo tenants, new tenants (`erp:tenant`) and, by migration, existing tenants; a tenant can add or relax rows. An event without a required dimension fails with `DIMENSION_MISSING` like a rule's own requirement. | `dimension_requirements` rows (`Accounting\Application\Posting\TenantDimensionRequirements`), migration `2026_09_30_000072_seed_dimension_requirements`. |
| A-191 | GA-10 | With the monthly statement run as the only payout path, how commission earned under a Phase 1 commission plan is paid is not specified: a statement whose accrued entries all come from Phase 1 plans (a plan id, no compensation scheme) is paid from the bank (COMMISSION_PAID), as the removed Phase 1 payout did; a scheme's commission keeps the A-22 route (payroll for an employee, else by producer type, default accounts payable); a producer on payroll is always paid through payroll. A statement the Phase 1 payout approved and nobody paid is listed in every month of the statement run so it can be paid. **Verify** with finance. | `config/erp.php` `distribution.plan_payout_route` (`ERP_PLAN_PAYOUT_ROUTE`, `ASSUMPTION:`), `CommissionStatementRun::route`, `StatementWorkbenchController::index`; `CommissionPayoutTest`. |
| A-192 | GA-10 | A-20 treats a Phase 1 plan as a flat scheme at accrual time but gives no rule to turn plans into schemes, so plans are not migrated: they keep accruing for product versions without a scheme (A-7, A-20), `/commission` shows them read-only (history) and new commission terms are set up as compensation schemes (the plan form on the screen is gone; the API `POST /api/insurance/commission-plans` stays for integrations). The Phase 1 agents list opens the producers register filtered to agents (`/distribution/producers?f.type=agent`), which keeps its own area permissions, so a branch officer (policy.create only) no longer has a producers list and picks producers through the quote's lookup. | `CommissionPageController`, `routes/web.php` (`agents` redirect), `navigation.ts`; `ClaimsCommissionApprovalsPagesTest`, `PartiesProductsPoliciesPagesTest`. |
| A-193 | GA-17 | Which forms require a customer's mobile number is not specified: an individual's mobile is required on the Parties form (new and edit); the quote's inline "New customer" drawer asks for it but does not require it, so a walk-in can be quoted first (the customer page flags a missing mobile). Mobiles are stored in international form: a Bangladeshi number typed as 01XXXXXXXXX, 8801… or +8801… becomes +8801XXXXXXXXX (operators 013–019); a number abroad must start with + and its country code. The identity number is an individual's NID or an organisation's BRN, unvalidated beyond 32 characters; a date of birth belongs to a person and is in the past; a contact person belongs to an organisation. Existing parties keep no contact details until edited. **Verify** with the branches. | `config/erp.php` `parties.mobile_required` (`ERP_PARTY_MOBILE_REQUIRED`, `ERP_QUOTE_CUSTOMER_MOBILE_REQUIRED`, `ASSUMPTION:`), `PartyContact`, `PartyPageController::contact`; migration `2026_10_03_000071`; `CustomerDetailsTest`. |
| A-194 | GA-38 | Agents collecting cheques and mobile-money payments is not specified beyond "agents collect": any receipt may name the collecting agent; only cash is owed by the agent (AGENT_CASH_COLLECTED to agent receivable, must be fully allocated, counted in agent cash and deposits); a cheque or mobile-money payment an agent collected reaches the company's bank and posts PREMIUM_RECEIVED like any receipt of its channel, and may leave part in suspense. | `ReceiptService::assertAgentCollection`, `CollectionsAccountingEvents::premiumReceived`, `AgentCashPositionQuery`; migration `2026_10_03_000072`; `AgentCashTest`. |
| A-195 | GA-38 | Who a receipt is "received from" when nobody says is not specified: the policyholder when every allocated installment belongs to one policyholder, otherwise nobody (a suspense receipt stays without a payer until one is chosen). The receipt form starts with the channel bank transfer, or — opened from a policy — the policyholder's last receipt channel (the user's own last channel no longer decides it). | `CollectionsPageController::store` / `singleHolder` / `prefill` (`ASSUMPTION:` A-195 comment), `receiptForm.ts`; `RecordReceiptAfterIssueTest`. |
| A-196 | GA-28 | Design OPEN 4 (credit rules) says nothing about cover notes: a cover note is issued on the same basis as the policy (A-117) — on credit only when the product version allows credit issue, otherwise the officer confirms the premium was received and gives its reference (1–128 characters), stored on the cover note (`issue_basis`, `premium_received_reference`) and audited; cover notes issued before this are marked `credit`. | `CoverNoteService::issue` (`PREMIUM_NOT_RECEIVED`), migration `2026_10_03_000074`, proposal page cover note drawer; `CoverNotesTest`. |
| A-197 | GA-31 | How a manual journal is referred to before it posts is not specified: the gapless JV number is still given only on posting (a rejected journal leaves no gap); until then the journal reads `MJ-` plus the last six characters of its id (upper case), shown on the journal page, the journals list and the page title with its status ("MJ-3F9A2C (pending approval)"). Not stored and not unique by construction (a collision within a tenant is very unlikely); search by it is not built. | `ProvisionalReference`, `JournalController` (`provisionalReference`), `journals/Show.vue`, `journals/Index.vue`; `ManualJournalVoucherTest`. |
| A-198 | GA-31 | Who attaches the supporting voucher of a manual journal is not specified: holders of `accounting.create_manual_journal` in the journal's entity, on manual and adjustment journals (not on system postings, refused as not found), with the journal when it is saved or later on its page, at any status (a voucher may arrive after posting); whoever may open journals downloads them. Documents are never removed (A-53). | `JournalPageController::ATTACH` (`ASSUMPTION:`), `ManualJournalPageController::store` (`voucher`); `ManualJournalVoucherTest`. |
| A-199 | GA-35 | Demo only, placeholders to verify: the Part A tenant gets a Chittagong branch (CTG) with a branch officer scoped to it (`branch.officer.ctg@<slug>.local`); the compensation scheme AGENCY-NL (agents only, non-life commission allowed per A-18, one rule: 10% of premium received) on every product instead of the Phase 1 plan; a fourth product FIRE-SP "Fire Short Period (seasonal stock)" with a one-month term, rated on the fire tariff, so a policy (sold in CTG on 3 Sep, expiring 2 Oct) can carry a renewal quotation offered on 12 Sep; a motor proposal with an active cover note (11 Sep–10 Oct, on credit); the opening bank balance credited to a new account 3000 Share Capital. The short-period premium is the annual tariff rate (no short-period scale). | `PartADemoSeeder` (`SECOND_BRANCH`, `coverNoteAndRenewal`); `PartADemoTest`, `RatingDemoSeedersTest`. |
| A-200 | GA-37, GA-30, GA-25 | The words and Bangla terms of `docs/glossary.md` (Producer, Customer/Policyholder, Quote/Quotation, Payment reminders, Commission statements, Claims incurred, Not renewed; প্রাপ্য প্রিমিয়াম, অনুপার্জিত প্রিমিয়াম রিজার্ভ, সংঘটিত ক্লেইম, বরাদ্দ…) are the team's reading of common Bangladeshi usage, **verify** with the insurer's staff; an expiry register row that ended without renewal reads "Expired, not renewed" (the stored status stays `lapsed`). Endorsement premium stays the full annual difference by default (A-119 unchanged: pro-rata is already one configuration switch, `erp.policies.endorsement_premium`, and changing the default would change every tenant's money). | `docs/glossary.md`, `resources/help/*.md`, `navigation.ts`, `status.ts`, `renewals/Index.vue`; `config/erp.php` `policies.endorsement_premium`. |
| A-201 | GA-05 | Who may run a nightly job now is not specified: a holder of `periods.soft_lock` (the close's own system tasks use it; the finance manager and CFO templates hold it), instead of a new permission. It runs synchronously for the signed-in tenant only, as the schedule would today, and is audited `job.run_now`. | `App\Http\Close\NightlyJobs` (`ASSUMPTION:`). |
| A-202 | GA-15 | When the next fiscal year may be opened is not specified: from the close screen, by the owner of the fiscal calendar (`periods.lock`, A-27), at most one year ahead — only once the latest year has started on the business clock (`FISCAL_YEAR_TOO_EARLY`) — as twelve open months from the day after the latest period, in every book of the latest year, never over existing periods or an existing year label (`FISCAL_YEAR_OVERLAP`). | `FiscalYearSetup::openNext` (`ASSUMPTION:`). |
| A-203 | GA-15 | How the year-end close is posted is not specified beyond "income and expense to retained earnings": the cumulative balance of every income and expense account at the year end (every earlier year closed the same way, so it is the year's result; a year never closed is closed with it), in one journal of kind `closing` dated the year end, posted at once by the finance manager running the close task (`periods.lock`), no separate checker; a rerun and the sign-off post only what changed since. Needs an account mapped to `retained_earnings` (`RETAINED_EARNINGS_UNMAPPED`). Verify with the customer's accountant. | `YearEndClose` (`ASSUMPTION:`). |
| A-204 | GA-43 | VAT and stamp duty are paid to the government by manual journal until AP exists (slice 2.3), without a policy dimension. The VAT and stamp duty registers are built from policy transactions, so they are compared with the payable's lines that carry a policy; lines without one (payments, manual corrections) are outside the register. The unearned premium reconciliation counts every line (A-61). | `PolicyDutyReconciler`, `AccountBalanceReconciler::excludesUnattributedLines` (`ASSUMPTION:`). |
| A-211 | GA-18 | The month a new tenant's fiscal year starts in by default is not specified: January. Bangladesh non-life insurers keep calendar-year accounts (annual accounts and IDRA returns run January–December); the hint offers July (the government's July–June year). The step still lets anyone pick another month before the year is opened. Verify with the customer. | `config/erp.php` `setup.fiscal_year_first_month` (env `ERP_FISCAL_YEAR_FIRST_MONTH`, `ASSUMPTION:`), `SetupPageController::show`. |
| A-212 | GA-18 | How a tariff template becomes a live tariff in a new tenant is not specified. The product step (needs `product.manage` and `rating.manage_plans`) creates, in one transaction: the product with the class's risk schema and coverages; the class's template duties (VAT and stamp duty, each narrowed to that class, skipped when a duty of that code is already in force for the class); the VAT tax rate; and the template tariff as a **draft** rating plan (an existing plan of that code is reused). The creator never approves it (R2 SoD, A-69): someone else holding `rating.approve_plans` (a CFO or another finance manager) approves and activates it in Tariffs. Until then the product cannot be rated; the step, the Done step and the flash name the tariff with a link. Template products are non-life, 12 months, monthly earning, VAT inclusive, no credit issue (A-117). All values stay placeholders flagged `verify`. | `SetupPageController::productFromTemplate` (`ASSUMPTION`), `TariffTemplates`, `ProductClassTemplates`, `setup/Index.vue`. |
| A-213 | GA-18 | The bank accounts and underwriting limits steps are not specified beyond "add them". Bank accounts: owned by `bank.manage_accounts` (Finance Manager); the GL account defaults to the account `bank_main` is mapped to; the currency is the entity's base currency; the step needs the company, fiscal year and chart first. Underwriting limits: owned by `underwriting.manage_limits` (Tenant Admin); accepts the A-90 placeholder defaults (flagged `verify`) from today, because a limit never starts in the past; a role and class that already has a limit keeps it. `UnderwritingLimits::acceptDefaults` now checks the permission even when nothing would be written. | `SetupWizard::STEPS`, `SetupPageController::bankAccounts` / `underwritingLimits`, `UnderwritingLimits::acceptDefaults`. |
| A-214 | GA-18 | The short code suggested for a company not saved yet is not specified: the initials of the words of its name, leaving out legal words (Ltd, Limited, PLC, Co, Company, The, And, Of, Pvt, Private, Inc); a one-word name gives its first four letters; at most 16 characters. The field follows the name until someone types their own. | `CompanySetup::suggestCode`, `resources/js/lib/setup.ts` `suggestCompanyCode`. |
| A-215 | GA-14 | When a cheque goes through clearing is not specified: a cheque receipt is taken into cheques in clearing when the entity maps the new account role `cheques_in_clearing` on the value date (`receipts.in_clearing`); otherwise it posts to the bank as before, so existing tenants keep their practice and history until they map the role (the chart template, the demo chart and new tenants have 1025 Cheques in Clearing). The whole cheque stays in clearing until someone records it cleared; a cheque that bounces before that leaves clearing, after that the bank. Agent collections stay cash only. | `ReceiptService::record` (`ASSUMPTION`), `AccountRoleMappingService::isMapped`, `CollectionsAccountingEvents`. |
| A-216 | GA-14 | The bank's charge for a returned cheque is not specified: optional on the bounce, booked as the company's expense on the new role `bank_charges` against the receipt's bank account (CHEQUE_RETURN_CHARGED), recorded on the receipt (`bounce_charge_minor`); recovering it from the customer is not built (verify). | `ChequeBounceService::bounce`, `CollectionsAccountingEvents::chequeReturnCharged`. |
| A-217 | GA-14 | Who records a cheque cleared is not specified: whoever may record its bounce (`receipt.allocate` on the receipt's branch). The clearing date cannot be before the value date. | `ChequeClearingService::clear` (`ASSUMPTION`). |
| A-218 | GA-14 | "Dunning level 1 at once" on a bounce is not specified further: every installment the bounce left unpaid on an issued or active policy gets its level-1 reminder dated the bounce (once per installment and level; the nightly run does not repeat level 1, level 2 follows the schedule), queued as `DunningNoticeDue` with reason `cheque_bounced`. The policy page banner and the Home queue "Policies with bounced premium" (branch officer, branch manager, accountant) list policies in force while such an installment is unpaid; lapse and cancellation stay with the existing grace rules (A-10). | `DunningRun::bouncedPremiumNotice` (`ASSUMPTION`), `BouncedPremiumQuery`, `WorkQueues`. |
| A-219 | GA-27 | Who records an unknown credit on the bank statement is not specified: the accountant who reconciles the bank. The accountant template gets `receipt.create` (existing tenants' accountant role by migration `2026_10_02_000502`); the receipt is a bank transfer into suspense on the branch chosen, dated and referenced from the line, and the line is matched to it once posted. "Post as bank charge" opens a manual journal prefilled from the line (DR `bank_charges` / CR the bank's GL account, line memo = statement text), which goes for approval as usual; journal lines without an event are referenced by their memo in bank matching. Ledger lines netting to zero are offset as one contra group (`bank.match`, undoable while their months are not locked). | `RoleTemplates`, `StatementLineReceiptController`, `ManualJournalPageController::prefill`, `BankMatcher::offset` / `undoOffset`, `AccountLineQuery`. |
| A-220 | GA-21 | Who receipts a claim recovery is not specified beyond "finance, like any inflow": the collections duties, `receipt.create` or `receipt.allocate` on the claim's branch, not the claims duties; payer (any active party) and bank account are required; the recovery gets a branch-coded receipt number `RCV-<branch>-<fy>-n`. The claim screen and the claims API use this path; `ClaimService::recover` stays for the claim lifecycle's reference-model tests. Verify with the customer whether claims staff may record recoveries. | `ClaimRecoveryReceipts` (`ASSUMPTION`), `ClaimPageController::recover`, `ClaimController::recover`, lookup `payer`. |
| A-221 | GA-26 | When a bank credit already "has a receipt" is not specified: when the matching screen would suggest a posted, unmatched debit of the same amount on the bank account's ledger account within the auto-match window (`erp.bank.auto_match_date_window_days`), or a receipt that is not bounced has the same amount and a reference of three or more characters that the line's reference or description quotes (on the same bank account, or none). Such lines leave "Receipts to record"; "Unmatched bank lines" still lists them for matching. | `WorkQueues::withoutReceipt` (`ASSUMPTION:`). |
| A-222 | GA-26 | Which role templates get the added Home queues is not specified: overdue premium, renewals due, cover notes ending and agent cash not deposited to the branch officer and manager (receipt.create, renewal.manage, cover_note.issue; a deposit needs receipt.create); referrals to the branch manager, finance manager and CFO (underwriting.decide, A-86); producer licences expiring to the branch manager (agent.manage); refunds to release to the finance manager and CFO (receipt.refund_release); commission to pay to the accountant (commission.pay, A-138). Customer role templates follow their own role codes, as the U6 queues do. | `WorkQueues::BY_ROLE`. |
| A-223 | GA-26 | The windows of the added queues are not specified: overdue premium is every unpaid installment of an issued or active policy due before today; renewals due are open expiry register entries expiring from today to the register's largest bucket (60 days by default); cover notes ending are active notes ending within `erp.cover_notes.expiring_within_days` (7); producer licences expiring are active licences ending within the earliest licence alert (`erp.distribution.licence_alert_days`, 60). | `WorkQueues::query`, `coverNoteDays`, `licenceDays`. |
| A-224 | GA-26 | "Referrals waiting for my decision" lists submitted referred proposals whose current approval step the user may decide now (the approvals inbox), limited to the user's branches; a referral is not counted twice in "Waiting for my approval". Approval steps are decided with the permission held tenant-wide (ApprovalService), so a branch-scoped manager has none — unchanged, found not changed. Refunds to release leave out refunds the user requested; commission to pay leaves out statements the user approved (maker ≠ checker). | `WorkQueues::approvals`, `query`. |
| A-225 | GA-29 | How a vehicle is searched is not specified: the registration and chassis numbers in the risk inputs of policies, proposals and quotations, compared on letters and digits only ("DHA GA 11-4321" finds DHA-GA-11-4321), for a query of four or more characters with a digit. Producers are found by code or name for those who open the producers area, tenant-wide like customers (A-160). A cover note opens its proposal (a cover note has no page of its own). | `GlobalSearchQuery::vehicleKey`, `vehicleOr`, `producers`, `coverNotes`. |
| A-226 | GA-40 | The list page size is not specified: `erp.ui.list_page_size`, 5,000 rows by default (the U10 page size), after which policies, claims, claim payments, receipts and parties are paged (`?page=`) with "Showing N of M" and the status bar pager; quotations, cover notes, renewals and referrals keep their first page and say how many there are; the allocation workbench lists 500 candidates, the refunds list the latest 100 and the commission history the latest 500, each with the count. Filters and totals on a paged list cover the page shown. | `PageSupport::listPageSize`, `partialNotice`. |
| A-227 | GA-16 | The phone layout starts below 640 px (Tailwind `sm`): the sidebar hides behind the ☰ button and opens over the page with its labels; the display settings menu is left out of the top bar there (theme and density stay in the command palette); the entity switcher shows an icon; the status bar hides the entity and branch. Desktop widths are unchanged. | `lib/phone.ts`, `TopBar.vue`, `Sidebar.vue`, `AppLayout.vue`. |
| A-231 | W7 (GA-24) | How small a balance may be written off is not specified: at most 1,000.00 of the policy's currency (`erp.premium_write_off.max_minor`, env `ERP_PREMIUM_WRITE_OFF_MAX_MINOR`, minor units); more is collected. Only a cancelled policy's whole unpaid balance is written off, never part of it and never an active policy's. | `config/erp.php` `premium_write_off.max_minor`, `PremiumWriteOffService::limitMinor` (`ASSUMPTION:`). |
| A-232 | W7 (GA-24) | Who writes off is not specified: the branch manager requests (`receipt.write_off_request`, the role that cancels and collects) and the finance manager and CFO approve (`receipt.write_off_approve`); no template holds both. Existing tenants get the grants by migration `2026_10_05_000702`. | `RoleTemplates` (`ASSUMPTION A-232`), `PermissionsSeeder`, `PermissionCatalogue`. |
| A-233 | W7 (GA-24) | A write-off always goes through the approval engine: an approval policy for `premium_write_off` (Approval limits screen) decides the steps when one matches the amount; without one, one step for `receipt.write_off_approve`. The final approval writes off what is still owed then (never more than requested) on the company's today; if the customer paid meanwhile, nothing is posted and the request is "settled". | `PremiumWriteOffService::request`, `completeApproval` (`ASSUMPTION:`). |
| A-234 | W7 (GA-24) | Where a write-off is charged is not specified: a new account role `premium_written_off` (expense "Premium Written Off", 5450 in the demo chart and the setup template), debited for the whole unpaid balance including its VAT and stamp duty, which stay owed to the government. Existing tenant books that map premium receivable get the account (5450, or the next free code) mapped from 2026-01-01 by migration `2026_10_05_000703`. | `resources/posting-rules/PREMIUM_WRITTEN_OFF.default.json`, `AccountRolesSeeder`, `DemoTenantSeeder`, `resources/setup/chart-of-accounts/non-life-insurance.csv`. |
| A-235 | W7 (GA-25) | Where a premium-free endorsement changes the details is not specified: on the policy (`policies.insured_details`: insured name, address, mortgagee, mobile, email), not on the customer record, so the customer's other policies keep their wording until endorsed too. Keys never endorsed come from the customer. An empty mortgagee removes it. Kinds: name, address, mortgagee, contact; period extension is not built (it changes premium). | `PolicyDetailsEndorsement` (`ASSUMPTION A-235`). |
| A-236 | W7 (L5) | The Bangla wording of the browser-side form checks and the browser's constraint messages is not specified: ours, mirroring the server's risk problems (A-161) with Latin digits (A-104). English keeps the browser's own constraint wording. | `resources/js/lib/validationMessages.ts`, `riskForm.ts` `problemMessage` (`ASSUMPTION A-236`). |
| A-241 | 2.3 | PD-3 OPEN (maximum list per event): a `for_each` line group repeats over at most 5,000 items (`JournalDraftBuilder::MAX_LINE_GROUP_ITEMS`); larger runs are split. | `JournalDraftBuilder` (`ASSUMPTION`). |
| A-242 | 2.3/2.4 | Role templates: accountant `ap.manage_suppliers`, `ap.enter_bills`, `ap.prepare_payments`; finance manager `ap.manage_suppliers`, `ap.approve_bills`, `ap.approve_payments`; CFO those plus `ap.release_payments`. Object SoD (audit-based) stops one person approving and releasing the same run. Existing tenants by migration `2026_10_04_000002`. | `RoleTemplates`, `PermissionsSeeder::SOD`. |
| A-243 | 2.3 | CQ-B3/B4 unanswered: VAT, VDS and TDS rates per supplier category in `erp.payables.categories` (rent 15/15/5 %, utility 5/0/0, telecom 15/0/0, repairs 10/10/5, professional 15/15/10, supplies 5/5/5, other 15/15/5), basis points of line net, half-even, VDS ≤ VAT. PLACEHOLDERS, verify with the tax adviser. | `config/erp.php`, `Withholding`. |
| A-244 | 2.3 | VAT on supplier bills is not recoverable (expensed with the line) unless `ERP_AP_INPUT_VAT_RECOVERABLE=true`, then DR `input_vat_receivable`. | `PayablesAccountingEvents`. |
| A-245 | 2.4 | CQ-I1 unanswered: bank payment file is a generic BEFTN-style CSV (Sl, payee, bank, branch, routing, account, amount, reference, value date); each download recorded append-only with SHA-256 in `bank_payment_files`. | `BankPaymentFile`. |
| A-246 | 2.3 | A bill posts on its bill date (accounting date), not on approval day. Bills get their BIL number at submission. | `BillService`. |
| A-247 | 2.3 | Whoever approves bills cancels posted ones (AP_BILL_CANCELLED mirror on the cancellation date); a draft is cancelled by its enterer. Payment runs are cancelled by preparer or approver before release. | `BillService::cancel`, `PaymentRunService::cancel`. |
| A-248 | 2.3 | Demo suppliers, TINs/BINs, routing and account numbers are invented placeholders. | `PayablesDemoSeeder`. |
| A-251 | G4 reinsurance | The statutory share of non-life business ceded to Sadharan Bima Corporation is a placeholder 50% (VERIFY with the insurer and IDRA/SBC): `erp.reinsurance.sbc_share_bp`, the default of each treaty's SBC share, editable per treaty. | `config/erp.php`, `ri_treaties.sbc_share_bp`. |
| A-252 | G4 reinsurance | The SBC share is applied first, to the gross sum insured and net premium; the treaty applies to the rest (the law's "share of reinsurance ceded" reading is not built). No state reinsurer set up → no compulsory share, noted on the policy. | `CessionEngine::targets`. |
| A-253 | G4 reinsurance | A policy's treaty is the active treaty for its product version's class (else its line of business mapped by `erp.reinsurance.lob_classes`) whose period covers the inception date; one active treaty per entity, class and underwriting year. | `CessionEngine::classOf`, unique index. |
| A-254 | G4 reinsurance | Sum insured = the latest endorsement re-rating's risk `sum_insured`, else the policy's frozen risk, else the proposal. Ceded premium follows the ceded share of sum insured on NET premium (no VAT/stamp duty); a policy without a sum insured cedes quota share/SBC on premium only. | `CessionEngine::sumInsuredOf`. |
| A-255 | G4 reinsurance | Commission is the treaty's % of ceded premium, the SBC share included; commission income is recognised at cession (not deferred). | `CessionEngine`, RI_PREMIUM_CEDED. |
| A-256 | G4 reinsurance | Changing a treaty applies to cessions computed afterwards; cessions are append-only movements (target − already ceded), so an endorsement cedes its change and a cancellation reduces every cession (facultative too) by the unearned share returned. Claims on a cancelled policy get no reinsurers' share. | `CessionEngine`, trigger `ri_cessions_append_only`. |
| A-257 | G4 reinsurance | Permissions: `ri.manage_treaties` finance manager + CFO; `ri.place_facultative` finance manager, CFO, branch manager; `ri.view` accountant (so finance manager, CFO), branch manager, auditor (read-only). No SoD rule named. Existing tenants by migration `2026_10_06_000251`. | `RoleTemplates`, migration. |
| A-258 | G4 reinsurance | A facultative placement may be made on any issued or active policy (not only above capacity); share, premium and commission are typed from the slip; the ceded sum insured defaults to the share of the whole sum insured; total ceded may not exceed the sum insured. No cancellation of a placement (reverse by manual journal). | `FacultativeService`. |
| A-259 | G4 reinsurance | Settlements with reinsurers (cash paid/received) are manual journals; lines without a reinsurer dimension are outside the reinsurer reconcilers. Recoverables become due when the claim payment is paid. | `ReinsurancePayableReconciler`, `ReinsuranceClaimsReconciler`, `ClaimCessions::onPaid`. |
| A-260 | G4 reinsurance | Reinsurer statements are per calendar quarter (Q3 = July–September), prepared any time (a quarter not yet ended shows to date) and refreshed on re-preparation, number `RIS-<fy>-n` kept. Demo reinsurer names "Global Re (placeholder)" and "Asia Pacific Re (placeholder)", ratings and treaty terms are placeholders. | `ReinsurerStatements`, `ReinsuranceDemoSeeder`. |
| A-261 | G5 | IDRA's return form numbers, wording and exact layouts are not known: forms are titled descriptively (premium income by class, claims by class, commission and management expenses against the limit, agent register summary, reinsurance ceded) and their sections and columns are configuration (`erp.regulatory.forms`). Verify each against the current IDRA returns (CQ-A2). | `config/erp.php` regulatory.forms, `ReturnFormBuilder`. |
| A-262 | G5 | Return periods are calendar quarters and the calendar year whatever the fiscal year; gross premium = premium written (new business and endorsements) without VAT and stamp duty, cancellations = unearned premium returned on cancellation, class = product line of business (A-60). A return is filed only after it is reviewed. | `RegulatoryPeriod`, `ReturnFormBuilder::premiumIncome`, `RegulatoryReturnService::file`. |
| A-263 | G5 | Management expenses = expense accounts not mapped to claims, IBNR, commission or rounding roles, spread over classes by gross premium; the IDRA expense limit is a placeholder 35% of gross premium per class (verify). | `erp.regulatory.expense_limit_bp`, `non_management_expense_roles`. |
| A-264 | G5 | IBNR percentage method: placeholder rates (fire 3%, marine 4%, motor 5%, other 5%, verify) on net written premium (excluding VAT, after cancellations) over the four quarters to the quarter end. | `erp.regulatory.provisions.ibnr_percentage_bp`, `TechnicalProvisionCalculator`. |
| A-265 | G5 | Paid chain ladder: claim payments by loss-date accident quarter and payment quarter, plus paid claims from before the system in `claims_paid_history` (legacy import; the demo seeds it); 8-quarter triangle, volume-weighted factors, no tail; needs paid claims in 4 accident quarters, else the class uses the percentage; IBNR = ultimate − paid − open case reserves, floored at 0. No import screen for the history yet. | `PaidChainLadder`, `RegulatoryDemoSeeder`. |
| A-266 | G5 | Premium deficiency check: unearned premium vs unearned premium × (expected loss ratio + 10% maintenance expenses), placeholder loss ratios per class; shown as a note, nothing posted. | `erp.regulatory.provisions.expected_loss_ratio_bp`. |
| A-267 | G5 | The IBNR provision posts per class (dimension `lob`) to the head office branch (`erp.regulatory.provisions.branch_code`, else the first branch), dated the quarter end; the prior posted run is released the same day by its own event. | `TechnicalProvisionService`. |
| A-268 | G5 | The finance manager prepares and reviews the technical provisions run (`provisions.run`) and marks returns filed (`regulatory.file`); the CFO approves and posts (`provisions.approve`), never a run they prepared or reviewed (SoD object rule provisions.run ✕ provisions.approve). Generating and reviewing returns needs `reports.regulatory`. | `RoleTemplates`, `PermissionsSeeder::SOD`, migration `2026_10_04_000101`. |
| A-269 | G5 | Solvency snapshot, placeholder formula: available = assets − liabilities; required = greatest of minimum paid-up capital BDT 40 crore, 20% of net premium and 30% of claims incurred over twelve months (verify with IDRA solvency rules). The demo company shows "below the requirement". | `SolvencySnapshot`, `erp.regulatory.solvency`. |
| A-270 | G5 | Close task "Technical provisions" appears only in a calendar quarter's last month once the entity has any technical provisions run; it blocks until that quarter's run is posted. A return's filing date lies between its period start and today. The reinsurance ceded form is an empty state until an `ri_cessions` table exists, then sums recognisable `*_minor` columns by class. | `TechnicalProvisionsCloseCheck`, `RegulatoryReturnService`, `ReturnFormBuilder::reinsuranceCeded`. |
| A-271 | §B.7 | Default asset classes and rates are not given: furniture 10 years and office equipment, leasehold improvements 5 years straight line, computers 3 years straight line, motor vehicles 20% a year reducing balance; residual 0; capitalisation from BDT 10,000. PLACEHOLDERS — verify with the insurer's finance team and auditors (tax rates in the Income Tax Act's schedule differ from book depreciation). | `config erp.fixed_assets.default_classes` (`ASSUMPTION`), `FixedAssetService::addDefaultClasses`, demo classes. |
| A-272 | §B.7 | Capitalisation is manual only (no AP bills yet): the credit goes to the bank account paid from or to `accounts_payable` when bought on credit; capitalising from AP bill lines through `asset_clearing` waits for slice 2.3. Disposals and transfers need no approval step in this MVP (one `fa.manage` holder records them, audited). | `FixedAssetService::acquire`, `dispose`, `transfer`. |
| A-273 | §B.7 | Depreciation starts in the month of acquisition (full month) and the month of disposal is not depreciated (design example); straight line absorbs rounding in the last month of the life; reducing balance = net book value × rate ÷ 12 each month. | `DepreciationCalculator` (`ASSUMPTION`). |
| A-274 | §B.7 | Assets owned before go-live are registered with their accumulated depreciation at the cut-over and post no event; their cost and accumulated depreciation come from an opening journal (PD-16). The demo cut-over is 31 July 2026 against retained earnings. | `FixedAssetService::registerOpening`, `FinanceModulesDemoSeeder`. |
| A-275 | §B.7 | A branch transfer posts FA_TRANSFERRED (cost and accumulated depreciation move between branch dimensions, no P&L) dated the move; depreciation of the month follows the branch the asset is in when the batch runs. | `FixedAssetService::transfer`, `DepreciationRun`. |
| A-276 | §B.7 | Permissions named `fa.manage` (classes, capitalise, move, dispose; accountant) and `fa.post_depreciation` (finance manager and CFO) instead of the design's `asset.*` codes, as the build request named them. | `PermissionsSeeder`, `RoleTemplates`, migration `2026_10_05_000271`. |
| A-277 | §B.8.1 | Budget granularity is account × branch × fiscal month (no cost centre: CQ-I9 open); the accountant prepares (`budget.prepare`), the finance manager and CFO approve (`budget.approve`, SoD object rule and preparer ≠ approver); budgets cover active income and expense accounts. | `BudgetService`, migration `2026_10_05_000272`. |
| A-278 | §B.6 | Petty cash duties: the branch manager is the float custodian and pays vouchers (`pettycash.spend`); the accountant asks for replenishment and counts (`pettycash.replenish`, never on a float they hold); the finance manager and CFO set up floats and approve replenishments (`pettycash.approve`; SoD replenish ✕ approve on the object, spend ✕ approve for the user). Replenishment is imprest: exactly the vouchers not yet replenished. | `PettyCashService`, migration `2026_10_05_000273`. |
| A-279 | §B.8.1 | The variance report and the CFO's Home block compare only accounts in the approved budget; income and expense outside it (claims incurred, commission, depreciation) are left out. Favourable = expense under budget or income over budget. | `BudgetVarianceQuery`, `WorkQueues::budgetBlock`. |
| A-280 | Demo | Fixed assets, budget amounts, petty cash payees and suppliers in the Part A demo are illustrative placeholders (Dhaka and Chattogram names), not customer data. | `FinanceModulesDemoSeeder`. |
| A-281 | People/Payroll | Monthly income tax deducted at source is not specified: (tax on the projected annual income from the slab table, at least the minimum tax when any tax is due) ÷ 12, half-even; projection = this month's regular earnings × 12 + the tax year's festival bonuses + taxable one-off inputs, less the exempt share (a fraction of income, capped). Investment rebate, arrears and months left in the year are LATER. Verify with the customer (CQ-J1). | `PayslipCalculator` (`ASSUMPTION`). |
| A-282 | People/Payroll | Which payroll month a commission paid through payroll joins is not specified: the month it was paid (its liability reached salary payable then) or its period end, whichever is later, moved to the first later month whose regular run is not posted. | `CommissionPayrollEarningConsumer` (`ASSUMPTION`). |
| A-283 | People/Payroll, CQ-F4 | Whether commission paid through payroll is taxed again as salary is OPEN (CQ-F4): not taxed again by default, because tax was withheld when it was earned; the payroll setting `commission_taxable` switches it. Flagged verify. | `payroll_settings.commission_taxable`, `PayslipCalculator`. |
| A-284 | People/Payroll | Bangladesh payroll values are placeholders flagged `verify`, seeded only in the demo: house rent 50% (G8 40%) and medical 10% of basic with a cap, fixed conveyance per grade; provident fund 10% + 10% of basic for permanent staff; festival bonus 100% of basic at each Eid after 6 months; FY2026-27 slabs 3,75,000 at 0%, 3,00,000 at 10%, 4,00,000 at 15%, 5,00,000 at 20%, 20,00,000 at 25%, rest 30%; one third of income exempt, capped at 5,00,000; minimum tax 5,000. Verify against the Finance Act and the service rules. | `PeopleDemoSeeder::configure`. |
| A-285 | People/Payroll, CQ-J3 | The salary bank file format is unknown: a CSV with a header (debit account, value date, employee code and name, bank, branch, routing number, full account number, amount, currency, reference) and a total row, stored encrypted per version on the run. | `PayrollRunService::bankFile`. |
| A-286 | People/Payroll | Payslip wording and Bangla labels are placeholders; the English PDF of a numbered payslip is kept in the document store on first print, Bangla is printed on demand. | `PayslipDocument`. |
| A-287 | People/Payroll | Who holds the payroll permissions is not specified: new template HR Manager (`hr.manage_employees`, `payroll.prepare`, `payroll.manage_rules`, `party.manage`); finance manager and CFO `payroll.approve`; accountant `payroll.pay`. SoD object rules `payroll.prepare` ✕ `payroll.approve`, `payroll.approve` ✕ `payroll.pay`. Existing tenants by migration `2026_10_04_000902`. | `RoleTemplates`, `PermissionsSeeder`. |
| A-288 | People/Payroll | Provident fund membership and proration are not specified: only employment types listed in the settings contribute (permanent); a joiner in the month is paid calendar days employed ÷ days in month of basic and allowances; PF on the prorated basic. | `PayslipCalculator`, `payroll_settings.pf_employment_types`. |
...
- **H2 (feedback): risk problems and reason messages in plain language** (D-60, A-161).
  - Every server path that validates risk inputs now names each field by the schema's label, in the user's language (`locale` preference, English or Bangla): the quotation live rating and save-and-issue / issue, the proposal "Enter risk details", the endorsement re-rating and the endorse-risk post (with its journal preview). "The risk details are not valid (chassis_no: REQUIRED)." now reads "Check the risk details: chassis number." at the top and "Enter the chassis number." at the field (Bangla: "ঝুঁকির বিবরণ দেখুন: চেসিস নম্বর।", "চেসিস নম্বর লিখুন।").
  - How: `RiskInputsInvalid` carries the schema that refused the inputs; `App\Http\Feedback\RiskProblems` words each problem (the English sentences are the browser's own, `riskForm.ts`). JSON refusals keep `errors` (codes, unchanged for API clients) and add `fields` (sentences) and a summary `message`; browser forms get `form`, `reason` and `risk_inputs.<key>`. A render for `RiskInputsInvalid` in `bootstrap/app.php` covers every browser and JSON path; `PreviewJournal` turns a JSON refusal into the same form errors. The quote workbench and the endorse drawer show the server's sentence per field.
  - Sweep: `tests/Unit/Feedback/ReasonMessagesCoverageTest` reads every reason code thrown in app/ (`new BusinessRuleViolation('X'`, `new RatingFailed('X'`, `parent::__construct('X'`, `new RiskSchemaInvalid(`, `PlanDefinitionInvalid::because(`; 4 sites with a variable code are listed with the codes they hold) and requires each to be worded by `ReasonMessages` or thrown with a written sentence without record ids, minor units, codes, snake_case keys, permission codes or passed-through technical text. It found 25 reasons; `ReasonMessages` now words them: fixed sentences (e.g. `DOCUMENT_PDF_FAILED`, `INVALID_RECOVERY_TYPE`, `PERIOD_NOT_OPEN`, `DUTY_NOT_FOUND`, `RISK_INPUTS_INVALID`), dates in words (`NOTHING_TO_PAY`, `PRODUCT_VERSION_NOT_EFFECTIVE`, `PRODUCT_VERSION_OVERLAP`), the formula named for rating expression errors, and set-up detail kept with keys and permission codes in words (`RATING_PLAN_INVALID`, `RISK_SCHEMA_INVALID`, `ROLE_CONFLICT` …). The JSON API keeps domain messages.
  - Test changes (requirement changed, stricter): `PolicyIssueFromProposalTest` expects the worded form and field errors (and the preview JSON and Bangla) instead of the raw message; `ProposalRiskDetailsTest` expects "Keep it to 32 characters." (the field's limit) instead of "This is too long.", plus the form summary and Bangla; `RoleAdministrationTest` and `UserAdministrationTest` expect permissions in words ("Platform: manage roles", "Cover note: issue") instead of their codes. New cases in `QuotationWorkbenchTest`; Vitest `risk-form.test.ts` +1.
  - Suite: 1,387 Pest tests (the full run's only 3 failures were those permission-code expectations, fixed and rerun), 349 Vitest tests.
  - Not changed: the browser's own checks before posting still word problems in English on a Bangla form.
- **H3 (claims): a reopened paid claim closes again and takes recoveries** (D-61, A-162; the 2.0d "found, not changed" item).
  - Before: a reopened claim returned to `reserved` even when paid; close needed a new approved payment (`INVALID_CLAIM_TRANSITION`) and recoveries were refused (`CLAIM_NOT_PAID`) until then, so rejecting was the only way out.
  - Now `ClaimService::close` also accepts a `reserved` claim with money already paid (only reachable by reopening). Nothing unsettled may remain (`PAYMENTS_OUTSTANDING` as before); the reserve set after reopening is released down to what was approved with the normal CLAIM_CLOSED journal (`close_release`), so Σ claims_outstanding per claim = 0 after close still holds.
  - `ClaimService::recover` accepts a recovery whenever anything was ever paid on the claim, whatever its status (§5.5 "recovery* (any time after paid)"). Every status that allowed one before (paid, closed) has a paid payment, so nothing accepted before is refused now.
  - The claim page offers Close and Record recovery on such a claim (`ClaimService::closable` / `recoverable`).
...
### Gap fixes W1 — roles, refunds, error pages, events, claims view — done
From docs/gap-audit.md (the findings table as the spec). Findings grouped per commit; full Pest, PHPStan, vue-tsc and Vitest green before each.

- **GA-01 (+ the refund part of GA-24)** → `bd6a8fe`. The branch manager requests refunds (A-171). After cancelling, the confirmation offers "Request the refund of X" (`/refunds?policy=` opens the request prefilled with everything refundable) or "Collect X" (receipt form prefilled; cancelled policies with earned premium unpaid are collectable, `NextSteps::COLLECTABLE_STATUSES`). "Cancel from" defaults to today. Journal line captions can differ by event (D-67): a cancellation's unearned premium debit reads "Premium for cover not given, taken off the bill or returned to the customer" (EN/BN).
- **GA-03** → `bd6a8fe`, DECISION D-65. The design split stays: a user without `receipt.allocate` in the branch records the receipt without allocation lines, noted for the policy (`receipts.for_policy_id`, migration `2026_09_30_000072`); the money waits in suspense, the confirmation says "your branch manager allocates it", the branch manager's Home has "Receipts to allocate" and the allocation workbench lists that policy first.
- **GA-13** → `bd6a8fe`. The accountant reads reports (A-172); Home shows "Journals I submitted" (draft, pending, rejected in the last 30 days) to anyone who cannot approve journals.
- **GA-08** → `bd6a8fe`. `/accounting/events`: failed events and events queued longer than 15 minutes (A-173), with event labels, the source record link and Requeue behind `accounting.requeue_event` for the finance manager and CFO (A-174, audited `accounting_event.requeued`). Home "Failed accounting events" links there, includes stale queued events and is also on the finance manager's and CFO's Home.
- **GA-07** → `7e88fa6` (fix(ux): GA-07, GA-23, GA-22). Browser 403/404/419/500/503 render `errors/Error` inside AppLayout (a bare card when signed out): title, `lang`, what the page needs (permission labels), the roles that give it, the administrators who can grant it, Back, "Ask for access" (mailto) and Home (D-66). JSON/API unchanged; with APP_DEBUG a 500 keeps the debug page; refused form posts still return to the form. Links gated with the target's permissions: Home "Receipts to record" opens `/receipts/create?statement_line=` (amount, date, reference and bank prefilled) for users without the bank area; the agent code links to the statement only with the commission area; the Referrals empty action only with the quotes area; the party page's bank-account form only with `party.manage` and its policy links only with the policy area; the journal source link only when the reader opens that screen (`JournalSources::linkFor`).
- **GA-23** → same commit. `PermissionDenied` is reported as a notice ("Permission denied", user id and permission) without a stack trace.
- **GA-22** → same commit. `PermissionCatalogue`: group labels and a one-line explanation for every permission (roles page and error page). The roles page shows an inline segregation notice when two ticked permissions break a user-level SoD rule (wildcards included). "Resend invitation" shows only while the invitation is open (no `user.password_set` since the last invitation; the password reset now audits it) and the status reads "Invited". The invite drawer picks a first role (tenant-wide, through RoleAssignmentService). The user timeline explains when roles predate the audit trail.
- **GA-12** → commit `fix(claims): GA-12, GA-20`. A read-only Policy panel on the claim (cover, sum insured from the proposal, premium, paid, still owed, unpaid by the date of loss, bounced cheques, other claims; the policy link only for people who may open it). Registration warns when premium due is unpaid (form and confirmation) and never refuses. `reports.claims` (A-175) opens outstanding claims, claims paid and loss ratio; the sidebar Reports item shows for claims roles.
- **GA-20** → same commit. The Part A demo admin stays Tenant Admin only, even locally (AdminUserSeeder adds the local browsing roles only in a tenant without its own finance manager), and every demo role assignment is audited so user timelines show it. **Not done:** branch-scoped branch officer and manager in the demo and a second branch (GA-35): the demo story's services and several screens (parties, printing, finance screens) still check permissions tenant-wide, so this waits for the branch filtering work (tried: `PartyService` refuses a branch-scoped manager during the story).

**Migrations:** `2026_09_30_000071` (refund request grant), `…000072` (`receipts.for_policy_id`), `…000073` (accountant reports and requeue grants), `…000074` (`reports.claims`).
**Tests changed because the requirement changed (stricter or equivalent):** `HomeQueuesTest` (branch manager "Receipts to allocate", accountant "Journals I submitted", finance manager and CFO "Failed accounting events", 9 queues for the three-role user); `RoleAdministrationTest` (the accountant role has 7 permissions, incl. `reports.financial`).
**Found, not changed:**
...
| Finding | What changed | Commit |
|---|---|---|
| GA-11 | Palette "New quote" and the Policies list open the rated quote workbench (`/quotations/create`, quotation.create); the typed-premium form is "New policy (unrated product)", offered on the Policies list only while a product has no rating plan (`unratedProducts`). | 659cb02 |
| GA-10 | One commission run (D-75): statements are prepared and approved only by the monthly statement run (web and API `commission-statements/prepare`, `/{id}/approve`), paid by someone else; the Phase 1 approve (service method, web and API routes, plan form) is gone and its tests were ported onto the run. `/commission` is read-only history (statements of both runs, Phase 1 plans). Plan-only commission is paid from the bank (A-191); plans are not migrated (A-192). One producer register (D-76): `/agents` redirects to `/distribution/producers?f.type=agent`, the agents page and form are gone. Sidebar: "Commission statements" (primary) replaces Commission, Statement run and Agents; palette "Approve or pay commission" opens the run. | 659cb02 |
| GA-35 | `erp:demo`: Chittagong branch with a branch-scoped officer; compensation scheme AGENCY-NL on the products (agents 10%); FIRE-SP one-month product and a CTG policy expiring 2 Oct 2026 with its renewal quotation; a motor proposal with an active cover note; opening bank balance credited to 3000 Share Capital; command text "2 branches, 4 products, 8 policies, a cover note, a renewal quote…" (A-199). `PartADemoTest` checks each. | 659cb02 |
| GA-37 | `docs/glossary.md` (EN/BN). Sidebar labels = page titles (Bank accounts, Month-end close, Compensation schemes, Cheque register, Payment reminders, Document templates, Commission statements, Renewals), no shared icons; "producer" for agent codes on the policy and forms; renewal rows "Expired, not renewed"; help and tour texts in the glossary's words (Claims incurred, receipts in suspense, producer, quote/quotation) in English and Bangla. | 659cb02 |
| GA-17 | Parties get mobile (stored +880…), email, address, NID/BRN, date of birth, contact person (migration `2026_10_03_000071`, columns on `parties`, RLS unchanged, no new table; D-77). Mobile required for a person on the Parties form, optional in the quote drawer (A-193). Customer lookup and Parties search by mobile and NID/BRN. The party page is an `ObjectPage` (Overview with contact details and bank accounts, Policies, Claims, Receipts, Timeline, Documents, Audit) with "Edit details" and "Add bank account" for party.manage only (the read-only roles no longer see the bank-account form). | 888a089 |
| GA-38 | Receipt: "Received from" (lookup; defaults to the single policyholder of the allocated installments, A-195), policy numbers link on the receipt page, payer and collecting agent shown and linked; "Collected by agent" for every channel — only cash is agent cash (migration `2026_10_03_000072`, A-194); the channel starts as bank transfer or the policyholder's last channel. | 888a089 |
| GA-30 | Language switch in the user menu (preference `locale`); the help panel's switch changes only the panel (`help_locale`, D-78); Bangla help and tour use the glossary's Bangla terms (no "unallocated রসিদ", "Premium receivable"…). | 888a089 |
| GA-25 | An endorsement's added installment carries its number (`installments.endorsement_no`, migration `2026_10_03_000073`) and reads `<policy>/E<n>` in receipts, lookups and the policy page; the confirmation names the endorsement and offers Record receipt (an increase) and Print endorsement (a second toast action, D-79). Mid-term pro-rata stays a configuration switch, default unchanged (A-200). Non-premium endorsement types not built. | bb5950d |
| GA-28 | A cover note needs the premium received with its reference unless the product issues on credit, like the policy (A-196; migration `2026_10_03_000074`); the cover notes queue shows the superseding policy and the premium basis. | bb5950d |
| GA-31 | Journal lines pick accounts with a searchable lookup (`/lookup/account`); a supporting voucher is attached with the journal and more on its page (DocumentStore subject `journal`, A-198); a provisional reference `MJ-XXXXXX` is shown while the journal waits and in the page title with its status (A-197). | bb5950d |

- **Migrations:** `2026_10_03_000071_add_contact_details_to_parties`, `2026_10_03_000072_record_the_collecting_agent_on_any_receipt`, `2026_10_03_000073_endorsement_number_on_installments`, `2026_10_03_000074_premium_basis_on_cover_notes`. No new tenant table, so no TenantIsolationEveryTableTest rows.
- **Permissions and role templates:** none changed. New checks use existing permissions (party.manage for party edit and documents, accounting.create_manual_journal for vouchers and the account lookup).
- **Assumptions and decisions:** A-191 to A-200, D-75 to D-79.
- **Shared files touched (additively):** `navigation.ts`, `commands.ts`, `ObjectPage.vue` (`extraTabs`, `hiddenTabs`), `toasts.ts`/`Toaster.vue` (`also` step), `LookupInput.vue` (customer contact fields), `TextInput.vue` (`tel`/`email` input modes), `NextSteps.php`, `config/erp.php`, `routes/web.php`, `routes/api.php`. `RoleTemplates`, `WorkQueues` and `ReportsPageController` not edited (the reports index still links "Commission statements" to `/commission`, now the history page; the Home block "Unallocated receipts" keeps its title, GA-37 would call it "Receipts in suspense").
- **Flow audit script (not edited, per the rules), run on commit 888a089 (demo seeded as `scripts/e2e.sh` does, the audit instead of the happy path):** steps 1–8, 10, 12 and 14 done; step 9 stopped because it clicks the first link named /Open|statement|matching/i on the bank page, which is the sidebar's "Commission statements" item (the accountant holds commission.pay since G3; the earlier "Statement run" item matched the same way), so the statement import, the close (11) and the lock (13) did not complete. The selector should be scoped to `main`. Step 10 selects journal accounts with `selectOption` on "Account, line N"; after bb5950d the account picker is a lookup (GA-31), so step 10 must type the account code and pick it from the list.
...
| GA | Commit |
|---|---|
| GA-05 nightly jobs visible and running | `2fd9a7c` |
| GA-15 next fiscal year and year-end close | `4d09be9` |
| GA-43 unearned premium, suspense, VAT and stamp duty reconciliations at close | `4d09be9` |
| GA-04 approvals inbox shows what approving posts | `4d09be9` |
| GA-09 claim reject and reopen, posting close tasks through the journal preview | `4d09be9` |

- **GA-05 (D-80, A-201):**
  - `job_runs` (migration `2026_10_02_000401_create_job_runs`, RLS) through `Platform\Jobs\JobRunLog`: every tenant's run of each nightly job (per entity for dunning) with status, times, who started it and the counts it returned, or the error. The jobs use `RunsNightly` (all tenants on the schedule, one with `forTenant()`).
  - Close screen toolbar: "Nightly jobs last ran …" (the most out-of-date job; "have not run yet" / "failed" when so) opens a drawer with each job, its schedule time, the last run (company clock) and what it did. "Run now" for `periods.soft_lock` holders, synchronous, this tenant only, audited `job.run_now` (`POST /close/jobs/{job}/run`).
  - `php artisan dev` (`composer dev`) runs a queue worker on posting,batch,recon,default and `schedule:work` (Horizon's default supervisor only watched `default`); `composer worker` runs the worker and the scheduler together. How to resume updated.
  - `php artisan erp:demo` runs policy start/expiry/earning, dunning and renewals once: demo policies are Active and POL-2's overdue installment has its reminder. The E2E still accepts "Issued or Active" and passed.
  - Also: three chart-of-accounts business-date reads through the BusinessClock (Architecture/BusinessClockTest was red on main).
- **GA-15 (D-81, A-202, A-203):**
  - "Open next fiscal year" on /close for `periods.lock` holders: `FiscalYearSetup::openNext` (same service as the wizard's first year), twelve months after the latest period, once the latest year has started, never over existing periods (`FISCAL_YEAR_TOO_EARLY`, `FISCAL_YEAR_OVERLAP`, `FISCAL_YEAR_MISSING`), audited `fiscal_year.opened` (`POST /close/fiscal-years`).
  - Year-end close task (`year_end_close`, order 12, `periods.lock`) only in the close run of a fiscal year's last month, after every task that posts to income and expense, before the trial balance: `YearEndClose` posts one `closing` journal dated the year end moving every income and expense balance to the `retained_earnings` account (`RETAINED_EARNINGS_UNMAPPED` without one), audited `fiscal_year.closed`; reruns and the sign-off post only what changed. Golden fixtures `tests/Fixtures/golden/year_end/` (loss and profit).
  - A-148: the demo tenant can now open FY 2027-28 from the close screen (today is inside FY 2026-27). The E2E date guard is unchanged: `erp:demo` still seeds only FY 2026-27 and does not roll the story's August–September dates forward, so after June 2027 the flow would post into a year the demo never opened unless someone opens it first.
- **GA-43 (D-82, D-83, A-204):** close tasks `upr_reconciliation` (7), `suspense_reconciliation` (9, the existing suspense reconciler), `vat_reconciliation` (10), `stamp_duty_reconciliation` (11) before the trial balance, each blocked on a variance like the others; the lock's variance check includes them. New reconcilers `UnearnedPremiumReconciler` (the F5 register), `PremiumTaxReconciler`, `StampDutyReconciler` implement `AccountBalanceReconciler` (account roles instead of `subledger_controls`). Pending documents at close (D-55) unchanged.
- **GA-04 (D-84):** the approvals inbox shows the step ("Step 2 of 2"), the request time on the company clock, a link ("Open the journal draft" / "Open the journal to reverse"), the facts (posting or reversal date, kind, reason) and the journal lines the final approval posts (manual journals; reversals mirrored). Approving previews the journal the decision writes (the preview now also lists journals written directly by the JournalWriter), so the last approval shows "These entries are posted", an earlier step "Nothing is posted yet"; the button reads "Review and approve".
- **GA-09:** claim Reject and Reopen go through `useMoneyForm` (Review → journal preview → Confirm; reopen under an approval policy previews nothing posted). Close tasks that post (premium earning, year-end close) show "Review and run" and the journal preview before running.
- **Result:** Pest 1,416 passed, PHPStan 0 errors, vue-tsc green, Vitest 383 passed, E2E happy path passed (after both commits).
- **Tests:** new `Close/NightlyJobsTest`, `Close/YearEndCloseTest` (with the golden year-end fixtures), `Close/CloseReconciliationTasksTest`, `Pages/ApprovalPreviewTest`, `Pages/PostingActionPreviewsTest`, a `Setup/PartADemoTest` case; vitest `nightly-jobs`, `approvals`. `tests/Support/FakePdfRenderer` replaces the anonymous class in `fakePdfRenderer()` (PHPStan intermittently reported its generics).
- **Existing tests changed (requirement changed; equivalent or stricter):** `MonthEndCloseTest` (the task list with the four reconciliations; the clean close runs them; accruals at index 7 over the API), `CloseReportsJournalsPagesTest` and `LockRefusesUnreconciledPeriodTest` (run the four reconciliations; 15 tasks), `WorkbenchScreensTest` (14 open tasks, trial balance at index 11), `SubledgerReconciliationTest` (seven reconcilers, their September balances, 28 nightly runs), `TenantIsolationEveryTableTest` (populates `job_runs`), `Architecture/DependencyTest` (JournalWriter also used by `YearEndClose`, D-81).
- **Not changed / follow-ups:**
...
| GA | Commit | What changed |
|---|---|---|
| GA-19 | b9188eb | Money drawers start at the company's today (`useBusinessToday`, the shared `businessToday`): statement run, refund pay, agent deposit (amount = undeposited cash), cheque bounce, producer advance and move, endorse and cancel (kept inside the cover), product versions, schemes, rule ends, new tariffs and duties. The payout preview lives on the W3 statement run (D-75 made it the only approve path): its draft shows entries, earned, tax withheld (now per entry too) and net before "Approve <net>"; the Phase 1 "Approve a payout" drawer and `GET /commission/statements/preview` were not ported. Agent cash looks the agent up (code · name). A Vitest sweep fails on any new empty "now" DateInput not listed with its reason. |
| GA-18 | b9188eb | Company code suggested from the name (A-214); fiscal year from January (A-211); new steps Bank accounts and Underwriting limits (A-213); product step starts from a tariff template — rated product, the class's duties, draft tariff approved by someone else (A-212, D-85); Done step says Saved / Not saved yet / Left for the <role> and lists tariffs waiting. `NewTenantFirstPolicyTest` walks a blank tenant (`erp:tenant`) to a rated motor policy with stamp duty paid into its bank. Template products earn `daily_365` (erp.setup.earning_method, GA-44). |
| GA-14 | 409a114 | Roles `cheques_in_clearing`, `bank_charges`; cheques wait in clearing when the role is mapped (A-215); rule v2 for the four receipt events + CHEQUE_CLEARED (D-86); "Cheque cleared" on receipt and register; bounce with optional bank charge (A-216, D-87), level-1 reminder at once (A-218), policy banner, Home queue "Policies with bounced premium". Migration `2026_10_02_000501`. |
| GA-27 | 409a114 | Offset ledger lines netting to zero (contra group, undo); "Record receipt" from an unknown credit into suspense with the line matched; "Post as bank charge" prefilled manual journal whose memo then matches; payout reference = statement number. Accountant gets `receipt.create` (A-219). Migration `2026_10_02_000502`. |
| GA-21 | 409a114 | `ClaimRecoveryReceipts`: recoveries receipted by `receipt.create`/`receipt.allocate` with bank account, payer and `RCV` number, on the claim page and the claims API; claims staff refused (D-88, A-220). ClaimService untouched. Migration `2026_10_02_000503`. |

- **Shared files edited (additive):** `WorkQueues` (bounced_premium), `RoleTemplates` (accountant receipt.create), `routes/web.php`, `LookupController` (`payer` type), `ReasonMessages`, `lib/status.ts`, `lib/events.ts`, posting rules (new versions and two new rules), `AccountRolesSeeder`, `DemoTenantSeeder`, chart template CSV, `roles.{en,bn}.md`.
...
| GA | Commit |
|---|---|
| GA-26 Home queues show real work | `69c0f83` |
| GA-29 command palette finds any number and offers the page's actions | `69c0f83` (policy page actions in `cc171d6`) |
| GA-40 tables on DataTable, paging and counts, lookups instead of whole-table selects | `cc171d6` |
| GA-16 phone width (400 px) | `cc171d6` |

- **GA-26 (A-221–A-224, D-90):**
  - "Receipts to record" leaves out credit lines the bank matching screen already suggests a posted receipt for, or that quote a receipt's reference (A-221).
  - New queues, per role template and branch-filtered with `within()`/AreaReach: Overdue premium (branch officer, manager; second block), Referrals waiting for my decision (branch manager, finance manager, CFO), Renewals due and Cover notes ending (branch roles), Agent cash not deposited (branch roles), Producer licences expiring (branch manager), Refunds to release (finance manager, CFO; not one's own request), Commission to pay (accountant; not a statement one approved). Each empty state is one sentence and one action.
  - "Approvals over threshold" is "Waiting for my approval" (referrals not counted twice). Queue links carry the list's own filter (`/claims?f.status=registered`, `/refunds?f.status=requested`, …).
  - Claim payments queue `GET /claims/payments` (open payments first), opened by "Payments to release" (`f.status=release_requested` for finance).
- **GA-29 (A-225, D-91):** search finds quotations, proposals and cover notes by number (short form too), producers by code or name, and policies, proposals and quotations by registration or chassis number typed with or without spaces and dashes — only in the user's areas and branches. Object pages register their allowed actions (`lib/pageActions.ts`): policy — record a receipt, endorse, renew, request the refund (new `actions.refund` flag and page button), cancel; proposal — submit, issue a cover note, issue the policy. The palette lists them first and resets the query every time it opens.
- **GA-40 (A-226, D-92):** policy installments, claim reserve history, targets and producer commission entries on DataTable (`:url-sync="false"`). Policies, claims, claim payments, receipts and parties are server-paged (`erp.ui.list_page_size`, default 5,000) with the status bar pager; DataTable shows "Showing N of M" for a page or a capped list (quotations, cover notes, renewals, referrals, allocation candidates, refunds, commission history, producer entries). Lookups replace the payer, new producer's party, refund policy and deposit agent selects (`/lookup/party`, `/lookup/refundable`, `/lookup/agent?for=agent-cash`).
- **GA-16 (A-227, D-93):** below 640 px the sidebar hides behind ☰ and opens over the page; top bar and status bar compact; object page headers stack; Home rows are cards; object tables scroll in their own container. `tests/e2e/phone-width.mjs` (run by `scripts/e2e.sh`) asserts `document.scrollWidth <= innerWidth` at 400 px on Home, a policy, receipt create/show and claim create/show, the menu, and the desktop sidebar. Screenshots (gitignored): `storage/w6/shots/{before,after2,final,desktop,palette}/`.
- **Migrations / permissions / role templates:** none.
- **Shared files touched:** `WorkQueues.php` (additive: new keys and match arms; `approvals()` takes the queue key), `GlobalSearchQuery.php`, `LookupController.php`, `PageSupport.php` (`listPageSize()`), `routes/web.php` (one route), `DataTable.vue`/`QueueView.vue` (`total` prop), `LookupInput.vue` (`params` prop), `AppLayout.vue`, `TopBar.vue`, `Sidebar.vue`, `StatusBar.vue`, `ObjectPage.vue`, `CommandPalette.vue`, `scripts/e2e.sh`.
- **Tests:** new `HomeRealWorkQueuesTest`, `GlobalSearchRecordsTest`, `ListPagingAndLookupsTest`, a GA-26 case in `BranchScopedQueuesTest`, Vitest `palette-page-actions`, E2E `phone-width.mjs`. Changed with the requirement (equivalent or stricter): `HomeQueuesTest` (role queue titles, indexes, 16 queues for three roles), `ErrorPagesTest` and `OfficerReceiptIntoSuspenseTest` (queue indexes after Overdue premium), `BranchScopedQueuesTest` and `CollectionsBankPagesTest` (refundable policies and agents checked through the lookups), `CancellationSettlementTest` (prefill carries the policy label), `PartiesProductsPoliciesPagesTest` (no `parties` prop).
- **Gates (per the lead's change of rules, no full Pest run):** targeted Pest 994 passed (tests/Architecture, tests/Unit, Home/queues, search, lookups, Flow, Distribution, Policies, Renewals, Claims, CoverNotes, Quotations, Underwriting, collections and page tests); PHPStan 0 errors; vue-tsc green; Vitest 428 passed; `npm run build` green; `phone-width.mjs` passed against the demo. An earlier full run on the GA-26/GA-29 code had 1,487 of 1,489 passing, the two failures being queue index shifts fixed in `69c0f83`.
- **Found, not changed:**
...
|---|---|---|
| L1 (GA-24) | "Write off small balance" on a cancelled policy: the whole unpaid balance up to `erp.premium_write_off.max_minor` (1,000.00, A-231), requested by the branch manager (`receipt.write_off_request`), always approved through the approval engine (`premium_write_off`: a matching approval policy, else one step for `receipt.write_off_approve`, finance manager and CFO; A-232, A-233). The final approval credits the installments and posts PREMIUM_WRITTEN_OFF (debit new role `premium_written_off` → 5450 Premium Written Off, credit premium receivable; golden fixture `04c`; A-234, D-96); audited on the policy; the approver sees the lines first. The installment lookup, allocation workbench and suspense options list cancelled policies' unpaid installments. | `4d20d71` |
| L3 (GA-04) | `EventLinesPreview` (D-95): the lines an event would post, without writing. Claim payment approval and release previews (claim, payee, date, lines); period reopening (month, status, journals in it, close signed off, reason, "posts nothing"). The manual journal page shows Approve/Reject — and on a reversal under an approval policy — to the holder of the current step (`ApprovalService::pendingOutlook`), deciding through `/approvals/{id}/decide` with the preview and `return_to`. | `4d20d71` |
| L2 (GA-25) | "Change details": name, address, mortgagee and contact endorsements on the policy (`policies.insured_details`, A-235), numbered `<policy>/E<n>`, zero deltas enforced by a check, no accounting event, printable (endorsement document shows before/now and wording; policy documents print the endorsed name, address, mortgagee), left out of the premium register (D-97). | `df5ac75` |
| L2 hint, L4, L5 (GA-25, GA-30) | No "At least 0.01" hint for a money minimum of one poisha. Help modules `distribution`, `refunds`, `cheques`, `agentcash`, `tariffs`, `users`, `limits`, `chart`, `events` (EN/BN) on their screens; close help covers the reconciliations, year-end close and nightly jobs. Bangla form checks: `lib/validationMessages.ts` (claim and unrated policy forms, browser constraint bubbles through one shell listener, D-98, A-236) and the risk form's messages in the user's locale. | `b6c0a89` |
| L6 | `scripts/flow-audit.mjs`: step 9 scoped to `main`; step 10 picks accounts in the lookup; step 11 loops over every task, confirms posting tasks' journal preview and passes over refused tasks; step 13 records the lock section (see below); step 14 as the finance manager. | `f076291` |

- **Migrations:** `2026_10_05_000701_create_premium_write_offs` (RLS; TenantIsolationEveryTableTest populates it), `…000702_grant_premium_write_off_permissions` (G3 pattern), `…000703_map_premium_written_off_account`, `…000704_premium_free_endorsements`.
- **Permissions and templates:** `receipt.write_off_request` (branch manager), `receipt.write_off_approve` (finance manager, CFO). Approval object type `premium_write_off` on the Approval limits screen.
- **Step 13 choice:** on 14 September nobody can lock September: the soft lock opens on the month's last day (30 Sep) for every role, the lock after month end, and a CFO's early lock with a reason (D-56) also needs the month soft-locked, so running it as the CFO is not a real path either. The script records the lock section and the note "hard lock opens on 1 Oct (CQ-C5)".
- **Flow audit run** (erp_test_b: `migrate:fresh --seed`, `erp:demo`, `serve --port=8775` and a queue worker, frontend built from this branch): all 14 steps done. Observations for the lead's re-measure (docs/flow-audit.md not edited): step 4 notes "No commission journal on the policy" although step 3's receipt journal shows the commission lines (the deferred accounting tab may need longer than the script's one-second wait); step 11 leaves Trial balance, Financial statements and Sign off open because of the soft-lock date.
- **Shared files touched (additively):** `routes/web.php`, `config/erp.php`, `RoleTemplates`, `PermissionsSeeder`, `PermissionCatalogue`, `AccountRolesSeeder`, `DemoTenantSeeder`, the setup chart CSV, `roles.<en|bn>.md`, `JournalSources`, `LookupController`, `CollectionsPageController`, `PolicyPageController`, `policies/Show.vue` (new drawers are components in `components/policies/`), `accounting/journals/Show.vue`, `AppLayout.vue` (one import and one line), `events.ts`, `help.ts`, `riskForm.ts`, and the `help` attribute on the AppLayout line of 20 pages.
- **Tests changed with the requirement:** `ChartOfAccountsScreenTest` (29 demo accounts), `HowThisWorksTest` (new modules and their screens, close help), `risk-form.test.ts` (no hint for a one-poisha minimum; a real minimum and a maximum still show). New: `PremiumWriteOffTest`, `ApprovalPreviewRemainderTest`, `PremiumFreeEndorsementTest`, `validation-messages.test.ts`.
- **Gates:** before the lead's rule change the full suite ran once on the final code: 1,490 of 1,492 passed; the two failures (chart account count, a write-off refusal message naming a record id) were fixed and their files rerun green. Final targeted run: tests/Architecture, tests/Unit, Flow, Policies, Help, approval previews, golden rules, chart of accounts, setup wizard, documents, reports — 904 passed. PHPStan 0 errors, vue-tsc green, Vitest 418 passed, `npm run build` green.
- **Not done / follow-ups:** a period-extension endorsement (it changes premium); pending write-offs are not listed among the documents that hold a period's lock (they post on the approval day, not in an earlier period); server-side Laravel validation messages (`lang/`) remain English only.

### Market gap G5 — regulatory returns and technical provisions (2026-09-14)
- **Built:** Regulatory dashboard (`/regulatory`: placeholder solvency snapshot, latest quarter's returns, latest provisions run); Regulatory → Returns (`/regulatory/returns`: period picker, five configurable forms from the registers and ledger, preview, generate → review → file with date and reference, audited; set or form as XLSX with a sheet per form or PDF through the Chrome renderer); Regulatory → Technical provisions (`/regulatory/provisions`: UPR by class, IBNR by percentage or paid chain ladder per class with the triangle and factors, premium deficiency notes, journal on posting; prepare/recalculate and review by the finance manager, approve and post by the CFO with the journal preview). Events IBNR_PROVISION / IBNR_PROVISION_REVERSED (golden fixtures `11`, `11b`), roles `claims_ibnr_expense`, `ibnr_provision`, permissions `regulatory.file`, `provisions.run`, `provisions.approve`, SoD rule, conditional close task `technical_provisions` (D-111). Assumptions A-261–A-270, decisions D-110–D-114 (DECISIONS.md).
- **Demo:** `erp:demo` posts the Q3 2026 run TPR-2026-000001 (motor and fire by chain ladder on legacy paid history, marine by percentage; total IBNR 243,711.78) and generates the Q3 returns with the agent and producer register summary filed (IDRA/NL/2026/Q3/AR-0147).
- **Shared files edited (additive):** `CloseTaskCatalogue`, `PeriodCloseService`, `AccountingServiceProvider` (conditional tasks), `XlsxWriter` (`workbookOfSheets`), `RoleTemplates`, `PermissionsSeeder`, `PermissionCatalogue`, `AccountRolesSeeder`, `DemoTenantSeeder` (accounts 2220, 5110), `PartADemoSeeder` (one line), `ReasonMessages`, `routes/web.php` (one require), `bootstrap/providers.php`, `navigation.ts`, `status.ts`, `events.ts`, `close/Run.vue`, `ClosePageController`, roles help.
- **Not done:** import screen for `claims_paid_history`; IDRA's real form numbering/wording; per-branch IBNR; reinsurance-net IBNR; annual returns beyond the same forms for a calendar year; Bangla PDF.


## People and Payroll MVP (addendum §B.9–§B.11)

Branch `worktree-agent-a31836bbce2fa82cb`. Assumptions A-281–A-288. Decisions:
- **D-120** Outbox consumers (PD-7): `Platform\Messaging\OutboxConsumer` (tagged) + `OutboxConsumers` relay (`OutboxConsumerRelayJob`, every minute, and in `erp:demo`); unconsumed types stay unrelayed. `CommissionPayrollEarning` is consumed by People; `CommissionPayableToAp` still waits for the AP module (TODO for the AP slice).
- **D-121** `producers.employee_id` gets no foreign key yet (existing data/tests carry ids without employees).
- **D-122** Employment changes (promotion, transfer, pay change) apply directly with audit; change requests with approval (§B.9.3) LATER.
- **D-123** Payroll rules are edited in place from an effective date (settings, structures) or replaced per tax year (slabs), audited; versioned rule sets with approval LATER.
- **D-124** The kernel has no `for_each` lines (PD-3 not built), so `PAYROLL_POSTED` v2 is one event per branch × department of a run (key `PAYROLL_POSTED:{run}:{branch}:{department}`, dims branch + `department`, `payroll_run`), with salary, festival bonus and employer PF expense; commission paid through payroll is not expensed again (credit salary_payable = net − pre-accrued). `PAYROLL_PAID` is one event per run. `COMMISSION_PAYOUT_TO_PAYROLL` v2 carries the employee on the salary payable line. Close tasks come from a new `CloseTaskContributor` contract (payroll tasks only for entities with employees).

Built: tables `departments, designations, grades, employees, employments, salary_structures, payroll_settings, payroll_tax_slabs, payroll_inputs, payroll_runs, payslips, payslip_lines, salary_bank_files` (migrations `2026_10_04_000901/000902`); role `bonus_expense`; permissions `hr.manage_employees, payroll.prepare, payroll.approve, payroll.pay, payroll.manage_rules`; `PayrollReconciler` (salary_payable) and close tasks `payroll_posted`, `payroll_reconciliation`; screens People → Employees (queue + page), Payroll runs (queue + workbench with journal preview), Payslips (PDF EN/BN), Salary structures and tax slabs. Demo (`PeopleDemoSeeder` from the Part A story): 30 employees HO/CTG, August posted and paid, Nasima Akter's August incentive paid through payroll and on her September payslip, September in preview. Golden fixtures 10b, 10c; test `tests/Feature/People/PayrollRunTest.php`.

Not done: loans/advances, final settlement, attendance/leave, self-service, change-request approvals, AP route consumer, payroll inputs screen beyond the run drawer, per-employee journal lines (needs PD-3).

### Reinsurance MVP (market gap G4) — done
Commit `feat(reinsurance)` on branch worktree-agent-aba21abc2c9307dfe. Assumptions A-251–A-260, decisions D-105–D-109.

- **Model:** migration `2026_10_06_000251_create_reinsurance_tables` — `reinsurers` (party role reinsurer, rating, country, SBC flag), `ri_treaties` + `ri_treaty_participants` (quota share / surplus, commission, SBC share), `ri_facultative_placements`, `ri_cessions` (append-only), `ri_policy_positions`, `ri_claim_shares`, `ri_upr_adjustments`, `ri_statements`; account roles `ri_premium_ceded`, `ri_payable`, `ri_commission_income`, `ri_unearned_premium`, `ri_outstanding_claims`, `ri_claims_recoverable` (demo COA 1410–1430, 2610, 4310, 5410; chart template); permissions `ri.manage_treaties`, `ri.place_facultative`, `ri.view`.
- **Engine:** `CessionEngine` listens to PolicyIssued/Endorsed/Cancelled; `ClaimCessions` to new Claims events `ClaimReserveChanged`/`ClaimPaid` (dispatched from `ClaimAccountingEvents`). Posting rules RI_PREMIUM_CEDED, RI_CLAIM_RESERVE_CEDED, RI_CLAIM_RECOVERABLE, RI_UPR_ADJUSTED with golden fixtures 11–13. Close tasks `ri_unearned_premium`, `ri_balances_reconciliation`, `ri_claims_reconciliation` (CloseTaskCatalogue).
- **Screens:** Reinsurance → Treaties (list, reinsurers, editor), Cessions (queue + above-capacity banner), Reinsurer statements (prepare Q drawer); policy page tab "Reinsurance" with the facultative drawer (journal preview); reports Premium bordereau / Claims bordereau (CSV/XLSX).
- **Demo (`erp:demo`):** SBC + Global Re / Asia Pacific Re (placeholders); FY2026 treaties motor QS 40%, fire surplus 5 crore × 9 lines, marine cargo QS 50%, SBC 50%; every story policy ceded as issued; claims shares on both claims; POL-HO-2026-000008 Meghna Knit Composite (150 crore, CFO-approved referral) with 25 crore placed facultatively (slip FAC/APR/2026/0917); August close runs the reinsurance tasks clean; Q3 2026 statements for SBC and Global Re.
- **Shared files edited (additive):** InsuranceServiceProvider, ClaimAccountingEvents, PartyService (`createReinsurer`), PolicyPageController (`reinsurance` prop), ReportsPageController (2 catalogue rows + match arm), CloseTaskCatalogue, ClosePageController, close/Run.vue, policies/Show.vue, navigation.ts, routes/web.php (require), RoleTemplates, PermissionsSeeder, PermissionCatalogue, AccountRolesSeeder, DemoTenantSeeder, PartADemoSeeder (2 lines), ReasonMessages, roles help en/bn, config/erp.php.
- **Tests changed:** MonthEndCloseTest (task list), PartADemoTest (one more customer/policy), PermissionsTest, RoleAdministrationTest, GoldenRulesTest (reinsurer dim), TenantIsolationEveryTableTest; new tests/Feature/Reinsurance/ReinsuranceCessionTest.
- **Not done:** non-proportional (XL) treaties; settlement of reinsurer balances and cash calls (manual journals); placement cancellation; commission deferral; cession recalculation on treaty change; PDF statement; statement approval; per-reinsurer bordereau filter; co-insurance.

## Slices 2.3/2.4 — Accounts payable (MVP, demo build)

- **Decisions:** D-100 the kernel gets `for_each` line groups (addendum §B.2.1 PD-3/PD-4): `item.*` in amounts/dims, `account: item.<field>` only for overridable roles (`bank_main`, `ap_expense`), checked active/postable/non-control; one event per bill and per run; golden fixtures 11, 11a, 11b. D-101 the paying bank account (bank, branch, routing, account name, encrypted number) lives on the supplier, not `party_bank_accounts` (BEFTN needs routing and name); second-person approval of bank changes is LATER. D-102 permission codes follow the build brief (`ap.manage_suppliers`, `ap.enter_bills`, `ap.approve_bills`, `ap.prepare_payments`, `ap.approve_payments`, `ap.release_payments`) instead of §B.4's names. D-103 `PayablesReconciler` is an AccountBalanceReconciler (D-82 pattern) on `accounts_payable` by `payee_party`, lines without a payee (commission to AP) outside; close task `ap_reconciliation` (order 11).
- **Built:** suppliers, bills (draft → pending_approval → posted → partially_paid/paid; cancel), approval engine object types `ap_bill`/`payment_run` (limit optional, else maker ≠ checker), payment runs (draft → pending_approval → approved → released; cancel), BEFTN CSV, bank line matching via the existing matcher, AP ageing report (reports index + export, reconciled to the ledger), supplier statement, Home queues Bills to approve / Payment runs to approve / Payment runs to release, sidebar Supplier bills, Payment runs, Suppliers.
- **Demo:** `PayablesDemoSeeder` from `PartADemoSeeder::story`: six Dhaka suppliers, six September bills (rent 85,000; garage bill on the motor collision claim; survey bill on the cargo claim; stationery waiting for approval), PRN-2026-000001 released by the CFO with file and matched statement line, PRN-2026-000002 approved and waiting for release.
- **Not done:** claim payments to a vendor payee becoming AP bills; bills from commission statements (outbox consumer PD-7); supplier bank change approval; editing a draft bill on screen (service exists); partial payment amounts in runs (runs pay the full outstanding); pending bills in the close's pending-documents check; withholding certificates/returns.
- **Shared files edited (additive):** `JournalDraftBuilder`, `AccountOverrides`, `PostingEngine`, `PostingRule`, `LineDimensions`, `ExpressionScope`, `AmountEvaluator`, `CloseTaskCatalogue`, `WorkQueues`, `RoleTemplates`, `PermissionsSeeder`, `PermissionCatalogue`, `ReasonMessages`, `ReportsPageController`, `LookupController` (account lookup for ap), `DemoTenantSeeder` + COA template (4 roles), `routes/web.php` (require), `navigation.ts`, `status.ts`, `events.ts`, close `Run.vue`; tests updated for the new close task (MonthEndClose, YearEndClose, LockRefusesUnreconciled, CloseReportsJournalsPages) and TenantIsolationEveryTableTest.

### Finance modules — fixed assets, budgets, petty cash (design addendum v2 §B.6–B.8) — done (MVP)
Three commits (`feat(fixed-assets)`, `feat(budgets)`, `feat(petty-cash)`). Assumptions A-271–A-280, decisions D-115–D-117.

| Module | What is built | Screens |
|---|---|---|
| Fixed assets (§B.7) | Asset classes with method, life or rate, residual, threshold and own GL accounts; manual capitalisation `FA_ACQUIRED`; opening register at cut-over; monthly batch preview → post `DEPRECIATION_POSTED` (one row per asset and month); disposal `FA_DISPOSED` with gain/loss; branch transfer `FA_TRANSFERRED`; register reconciliation per GL account; conditional close tasks `depreciation` and `fixed_asset_reconciliation` | /fixed-assets (queue, capitalise), /fixed-assets/{id} (schedule, movements, documents, accounting, Move, Dispose), /fixed-assets/classes, /fixed-assets/depreciation, /fixed-assets/register (+ export) |
| Budgets (§B.8.1) | Versions draft → submitted → approved → superseded (preparer ≠ approver, DB trigger keeps approved lines), grid account × month per branch, paste from a spreadsheet (tab-separated), copy last year's actuals ± %, new version; variance month and YTD by group and branch (red/green) with export; Home block "Expense vs budget this month" (CFO, finance manager) | /budgets, /budgets/{id}, /budgets/variance (+ export, on /reports) |
| Petty cash (§B.6) | Floats per branch (custodian, limit, GL account) `PETTY_CASH_FLOAT_ISSUED`; vouchers with receipt photo `PETTY_CASH_SPENT` (refused above cash on hand); replenishment request → approval `PETTY_CASH_REPLENISHED`; surprise count `PETTY_CASH_COUNT_DIFFERENCE`; petty cash book with export | /petty-cash, /petty-cash/{id}, /petty-cash/book (+ export, on /reports) |

- **Shared files edited (additive):** `CloseTaskCatalogue` + `PeriodCloseService` + `AccountingServiceProvider` (conditional tasks, D-115), `ReportsPageController::index` (config `erp.reports.screens`), `reports/Show.vue` (`baseUrl`), `Sidebar.vue` + `navigation.ts` (group "Assets & budgets"), `WorkQueues`, `status.ts`, `PermissionsSeeder`, `RoleTemplates`, `PermissionCatalogue`, `ReasonMessages`, `AccountRolesSeeder`, `DemoTenantSeeder` (role accounts 1040, 1600, 1690, 4800, 5480, 5490, 5700), COA template CSV, `roles.*.md`, `config/erp.php` (overridable roles, reports screens, default classes), `PartADemoSeeder` (one line), `ClosePageController`/`close/Run.vue` task names, isolation test rows.
- **Migrations:** `2026_10_05_000271_create_fixed_assets`, `…272_create_budgets`, `…273_create_petty_cash` (each grants its permissions to existing tenants' template roles and adds SoD rules).
- **Verified:** `migrate:fresh --seed` + `erp:demo` on a fresh database: 25 assets (one disposed), August depreciation posted by the August close (22 assets), both FA close tasks done, September previewable (24 assets); FY2026 budget approved with September variances; two floats, 12 vouchers, one paid replenishment; every new page renders (200) for the demo roles. PHPStan on the new code 0 errors; `vite build` and `vue-tsc` green.
- **Not run (lead's instruction for the demo):** Pest. Tests written but not run: `tests/Feature/Finance/FixedAssetsTest.php`, golden fixtures `11*_fa_*.json`; the petty cash rules have no golden fixture yet.
- **Not done:** capitalisation from AP bills (asset clearing), disposal approval step, asset documents at capitalisation (attach on the asset page after), budget import wizard and cost centres, petty cash voucher void, petty cash reconciler and close check, expense claims (need employees), the kernel reconciliation screen for FA (the register report shows it).
